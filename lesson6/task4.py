...
class WorkCar(Car):
    def show_speed(self):
        super().show_speed()
        if self.speed > 40:
            print(f'Внимание! Ваша скорость - {self.speed} км/ч. Это слищком быстро, едьте помедленней!!')
...
town_car = TownCar(62, 'синяя', 'Porche', False)
work_car = WorkCar(39, 'белая', 'BMW', False)
sport_car = SportCar(160, 'красная', 'Ferrari', False)
police_car = PoliceCar(90, 'черная', 'Lada Kalina', True)
