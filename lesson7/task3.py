...
class Cell:
    ...
    def __add__(self, other):
        return str(f'Сумма клеток: {self.number + other.number}')

    def __sub__(self, other):
        if self.number > other.number:
            return str(f'Разность клеток: {self.number - other.number}')
        else:
    ...
    def __truediv__(self, other):
        if other.number != 0:
            return str(f'Результат деления: {round(self.number / other.number, 2)}')
        else:
            return 'Деление невозможно'

    def make_order(self, count):
        return '\n'.join(['*' * count for _ in (range(self.number // count))]) + '\n' + '*' * (self.number % count)
